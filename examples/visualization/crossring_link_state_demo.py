...
def create_demo_crossring_model():
    ...
    for node_id in range(4):
        ...
        # 通道缓冲区
        node.channel_buffer = {
            'gdma': create_demo_fifo(),
            'ddr': create_demo_fifo()
        }
        ...
        # IP接口
        node.ip_interfaces = {
            'gdma': create_demo_ip_interface(),
            'ddr': create_demo_ip_interface()
        }
    ...
    # 创建链路
    link_configs = [
        ('h_0_1', 0, 1), ('h_2_3', 2, 3),  # 水平链路
        ('v_0_2', 0, 2), ('v_1_3', 1, 3)   # 垂直链路
    ]
...
def create_demo_fifo():
    ...
    for i in range(random.randint(0, 3)):
        flit = _FlitProxy(
            ...
            fid=f"F{i}",
            etag=random.choice(["T0", "T1", "T2"]),
            ih=random.random() < 0.1,
            iv=random.random() < 0.1
        )
...
def create_demo_ip_interface():
    ...
    # L2H FIFOs
    ip.l2h_fifos = {
        'req': create_demo_fifo(),
        'rsp': create_demo_fifo(),
        'data': create_demo_fifo()
    }
...
def demo_animated():
    ...
    try:
        for cycle in range(100):
            ...
            if cycle % 10 == 0:
                node_count = sum(1 for node in demo_model.nodes.values() 
                               for fifo in node.inject_direction_fifos.values() 
                               if fifo.queue)
                link_count = sum(1 for link in demo_model.links.values()
                               for slice_obj in link.slices
                               if slice_obj.slot and slice_obj.slot.valid)
...
def main():
    ...
    choice = input("输入选择 (1-2, 默认1): ").strip() or "1"
