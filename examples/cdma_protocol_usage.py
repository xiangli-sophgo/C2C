...
class CDMAUsageExample:
    ...
    def example_1_basic_data_transfer(self):
        ...
        recv_result = compute_a.cdma_receive(
            dst_addr=0x10000000, dst_shape=(1024, 512), dst_mem_type=MemoryType.GMEM, src_chip_id="master", data_type="float32"  # 目标地址  # 模型参数形状  # 全局内存
        )
        ...
        # 2. 主控芯片发送数据
        print("\n步骤2: 主控芯片发送模型参数")
        send_result = master.cdma_send(src_addr=0x20000000, src_shape=(1024, 512), dst_chip_id="compute_a", src_mem_type=MemoryType.GMEM, data_type="float32")  # 源地址  # 数据形状
    ...
    def example_2_tensor_operations(self):
        ...
        # 1. 主控芯片准备接收聚合后的梯度
        print("\n步骤1: 主控芯片准备接收聚合梯度")
        recv_result = master.cdma_receive(dst_addr=0x30000000, dst_shape=(512, 256), dst_mem_type=MemoryType.GMEM, src_chip_id="compute_a", data_type="float32")  # 梯度张量形状

        # 2. 计算芯片A发送梯度（使用sum reduce）
        print("\n步骤2: 计算芯片A发送梯度（求和聚合）")
        send_result_a = compute_a.cdma_send(src_addr=0x40000000, src_shape=(512, 256), dst_chip_id="master", src_mem_type=MemoryType.GMEM, data_type="float32", reduce_op="sum")  # 求和聚合

        # 3. 计算芯片B也发送梯度
        print("\n步骤3: 计算芯片B发送梯度（求和聚合）")
        send_result_b = compute_b.cdma_send(src_addr=0x50000000, src_shape=(512, 256), dst_chip_id="master", src_mem_type=MemoryType.GMEM, data_type="float32", reduce_op="sum")
    ...
    def example_3_memory_hierarchy(self):
        ...
        # 计算芯片准备接收到L2缓存
        recv_l2_result = compute_a.cdma_receive(dst_addr=0x60000000, dst_shape=(256, 128), dst_mem_type=MemoryType.L2M, src_chip_id="storage", data_type="int32")  # L2缓存

        # 存储芯片从全局内存发送
        send_gmem_result = storage.cdma_send(src_addr=0x70000000, src_shape=(256, 128), dst_chip_id="compute_a", src_mem_type=MemoryType.GMEM, data_type="int32")  # 全局内存
        ...
        recv_lmem_result = compute_a.cdma_receive(
            dst_addr=0x80000000, dst_shape=(128, 64), dst_mem_type=MemoryType.LMEM, src_chip_id="compute_a", data_type="int32"  # 处理后的数据更小  # 本地内存  # 芯片内部传输
        )

        send_l2_result = compute_a.cdma_send(src_addr=0x60000000, src_shape=(128, 64), dst_chip_id="compute_a", src_mem_type=MemoryType.L2M, data_type="int32")
        ...
        return all([recv_l2_result.success, send_gmem_result.success, recv_lmem_result.success, send_l2_result.success])
    ...
    def example_4_concurrent_operations(self):
        ...
        def transfer_task(src_chip, dst_chip, src_addr, dst_addr, shape, task_name):
            ...
            try:
                # 目标芯片准备接收
                recv_result = self.chips[dst_chip].cdma_receive(dst_addr=dst_addr, dst_shape=shape, dst_mem_type=MemoryType.GMEM, src_chip_id=src_chip, data_type="float32")

                # 源芯片发送数据
                send_result = self.chips[src_chip].cdma_send(src_addr=src_addr, src_shape=shape, dst_chip_id=dst_chip, src_mem_type=MemoryType.GMEM, data_type="float32")
