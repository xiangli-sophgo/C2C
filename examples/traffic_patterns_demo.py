...
class TrafficGenerator:
    ...
    def setup_chip_topology(self):
        ...
        chip_types = {
            "master": "主控芯片",
            "compute_0": "计算芯片0", "compute_1": "计算芯片1", 
            "compute_2": "计算芯片2", "compute_3": "计算芯片3",
            "storage_0": "存储芯片0", "storage_1": "存储芯片1",
            "io_chip": "IO芯片"
        }
        ...
        while time.time() - start_time < duration:
            ...
            for target in broadcast_targets:
                try:
                    ...
                    # 主控芯片发送数据
                    send_result = self.chips["master"].cdma_send(
                        src_addr=0x20000000,
                        src_shape=(512, 256),
                        dst_chip_id=target,
                    ...
                    if success:
                        self.metrics[pattern].update(
                            send_result.bytes_transferred,
                            send_result.throughput_mbps,
                ...
                for i, compute in enumerate(compute_chips):
                    try:
                        ...
                        recv_result = self.chips[compute].cdma_receive(
                            ...
                            dst_shape=(128, 64, 4),  # 3D数据批次
                            dst_mem_type=MemoryType.L2M,  # 使用L2缓存
                            src_chip_id=storage,
                            data_type="float32"
                        )
                        ...
                        send_result = self.chips[storage].cdma_send(
                            src_addr=0x60000000 + i * 0x100000,
                            src_shape=(128, 64, 4),
                            dst_chip_id=compute,
                            src_mem_type=MemoryType.GMEM,
                            data_type="float32"
                    ...
                    if src != dst:
                        try:
                            ...
                            recv_result = self.chips[dst].cdma_receive(
                                dst_addr=0x70000000 + hash(src + dst) % 0x1000000,
                                dst_shape=(64, 32),
                            ...
                            send_result = self.chips[src].cdma_send(
                                src_addr=0x80000000 + hash(dst + src) % 0x1000000,
                                src_shape=(64, 32),
                                dst_chip_id=dst,
                                src_mem_type=MemoryType.LMEM,
                                data_type="int32"
                            )
    ...
    def generate_pipeline_traffic(self, duration: float = 4.0):
        ...
        while time.time() - start_time < duration:
            ...
            try:
                # 准备接收
                recv_result = self.chips[dst].cdma_receive(
                    dst_addr=0x90000000 + stage_index * 0x100000,
                    dst_shape=(256, 128),
                    dst_mem_type=MemoryType.GMEM,
                    src_chip_id=src,
                    data_type="float32"
                ...
                # 发送数据
                send_result = self.chips[src].cdma_send(
                    src_addr=0xA0000000 + stage_index * 0x100000,
                    src_shape=(256, 128),
                    dst_chip_id=dst,
                    src_mem_type=MemoryType.GMEM,
                    data_type="float32"
    ...
    def generate_performance_report(self):
        ...
        for pattern in TrafficPattern:
            ...
            if metrics.total_operations > 0:
                print(f"{pattern.value:<12} {metrics.total_operations:<8} "
...
def main():
    ...
    # 创建8芯片系统
    chip_ids = ["master", "compute_0", "compute_1", "compute_2", "compute_3", 
                "storage_0", "storage_1", "io_chip"]
