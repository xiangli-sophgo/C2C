...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler('crossring_demo.log')
        ]
    )
...
def test_crossring_flit():
    ...
    try:
        from noc.crossring.flit import create_crossring_flit, return_crossring_flit, get_crossring_flit_pool_stats
...
def test_crossring_config():
    ...
    try:
        from noc.crossring.config import (
        ...
        # 测试自定义配置
        custom_config = create_crossring_config_custom(
            3, 3, "test_3x3",
            burst=8,
            gdma_bw_limit=256.0
        )
...
def test_crossring_ip_interface():
    ...
    try:
        from noc.crossring.config import create_crossring_config_custom
        from noc.crossring.ip_interface import CrossRingIPInterface
...
def test_crossring_model():
    ...
    try:
        from noc.crossring.model import create_crossring_model
...
def test_integration():
    ...
    try:
        ...
        # 快速启动仿真（小规模测试）
        print("启动快速仿真测试...")
        results = quick_start_simulation(
            config_name="custom",
            max_cycles=1000,
