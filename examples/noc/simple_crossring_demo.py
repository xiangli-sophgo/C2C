...
def run_crossring_simulation(rows=2, cols=3, max_cycles=1000, max_requests=10):
    ...
    try:
        ...
        # 2. 注入流量并运行仿真
        injected = model.inject_from_traffic_file(traffic_file_path=str(traffic_file), max_requests=max_requests, cycle_accurate=True)  # 使用周期精确模式
        ...
        if bandwidth.get("total_bytes", 0) > 0:
            ...
            print(f"  带宽: {bandwidth.get('gbps', 0):.4f} Gbps")

        if basic.get("completion_rate", 0) < 100:
            print(f"\n⚠️  部分请求未完成: {basic.get('completed_requests', 0)}/{basic.get('total_requests', 0)}")
        else:
            print("\n✅ 所有请求完成")
