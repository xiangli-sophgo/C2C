...
def run_basic_simulation():
    ...
    simulator.add_cdma_send_event(
        timestamp_ns=5000,
        source_chip_id="chip_1",
        target_chip_id="chip_0",
        data_size=2048
    )
...
def run_complex_simulation():
    ...
    for i in range(4):
        next_i = (i + 1) % 4
        simulator.add_periodic_traffic(
            source_chip_id=f"chip_{i}",
            target_chip_id=f"chip_{next_i}",
            period_ns=25000,  # 25微秒周期
            data_size=1024,
            start_time_ns=i * 5000,  # 错开启动时间
            end_time_ns=500000  # 500微秒
        )
    
    # 添加跨芯片通信（chip_0 <-> chip_2）
    simulator.add_periodic_traffic(
        source_chip_id="chip_0",
        target_chip_id="chip_2", 
        period_ns=50000,  # 50微秒周期
        data_size=2048,
        start_time_ns=10000,
        end_time_ns=400000
    )
