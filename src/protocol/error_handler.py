...
class ErrorDetector:
    ...
    def register_packet_received(self, packet_id: str, sequence_number: int, 
                                source: str, checksum_valid: bool) -> List[ErrorRecord]:
        """
        注册包接收并检测错误
...
class RetransmissionManager:
    ...
    def schedule_retransmission(self, sequence_number: int, data: bytes, 
                              destination: str, packet_id: str) -> str:
        """
        调度重传
...
class ErrorHandler:
    ...
    def handle_error(self, error_record: ErrorRecord) -> RecoveryAction:
        ...
        with self._lock:
            ...
            # 确定恢复策略
            recovery_action = self._recovery_strategies.get(
                error_record.error_type, RecoveryAction.IGNORE
            )
    ...
    def _execute_recovery_action(self, error_record: ErrorRecord, 
    ...
    def _handle_retransmission(self, error_record: ErrorRecord) -> bool:
        """处理重传"""
        if (error_record.sequence_number is not None and 
            'data' in error_record.context and 
            'destination' in error_record.context):
            
            self._retransmission_manager.schedule_retransmission(
                sequence_number=error_record.sequence_number,
                data=error_record.context['data'],
                destination=error_record.context['destination'],
                packet_id=error_record.packet_id or ""
            )
            return True
        return False
    ...
    def process_packet_sent(self, packet_id: str, sequence_number: int, 
                          destination: str, data: bytes):
        """处理包发送"""
        self._error_detector.register_packet_sent(packet_id, sequence_number, destination)
    
    def process_packet_received(self, packet_id: str, sequence_number: int,
                               source: str, data: bytes, metadata: Dict[str, Any] = None) -> List[ErrorRecord]:
        """
        处理包接收
        ...
        # 错误检测
        errors = self._error_detector.register_packet_received(
            packet_id, sequence_number, source, integrity_valid
        )
