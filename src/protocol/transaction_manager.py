...
class TransactionManager:
    ...
    def create_paired_transaction(self, send_chip_id: str, recv_chip_id: str, 
                                transaction_id: str, timeout_seconds: float = 30.0) -> TransactionInfo:
        """
        创建配对传输事务
        ...
        with self._lock:
            ...
            transaction = TransactionInfo(
                ...
                send_chip_id=send_chip_id,
                recv_chip_id=recv_chip_id,
    ...
    def register_receive_operation(self, transaction_id: str, dst_addr: int, 
                                 dst_shape: Tuple[int, ...], dst_mem_type: str,
                                 data_type: str = "float32") -> bool:
        """
        注册CDMA_receive操作
    ...
    def register_send_operation(self, transaction_id: str, src_addr: int,
                              src_shape: Tuple[int, ...], src_mem_type: str,
                              data_type: str = "float32") -> Optional[TransactionInfo]:
        """
    ...
    def _validate_compatibility(self, transaction: TransactionInfo, 
                              src_shape: Tuple[int, ...], data_type: str) -> bool:
        """验证源和目标的兼容性"""
        # 检查形状是否匹配
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            active_transactions = len([t for t in self._transactions.values() 
                                     if t.state in [TransactionState.CREATED, 
                                                   TransactionState.RECEIVE_POSTED,
                                                   TransactionState.SEND_EXECUTED,
                                                   TransactionState.TRANSFERRING]])
    ...
    def clear_completed_transactions(self):
        ...
        with self._lock:
            completed_transaction_ids = [
                tid for tid, transaction in self._transactions.items()
                if transaction.state in [TransactionState.COMPLETED, TransactionState.FAILED, TransactionState.TIMEOUT]
            ]
