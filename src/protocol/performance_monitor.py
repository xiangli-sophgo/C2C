...
class ThroughputCounter:
    ...
    def _update_throughput_stats(self):
        ...
        # 计算最近1秒的字节数
        recent_bytes = [self._byte_counts[i] for i, ts in enumerate(self._event_timestamps) 
    ...
    def get_throughput_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            for window in windows:
                recent_events = [ts for ts in self._event_timestamps 
            ...
            for category, timestamps in self._categorized_events.items():
                ...
                category_stats[category] = {
                    'tps_1min': len(recent_category_events) / 60,
...
class MetricsCollector:
    ...
    def __init__(self, source_id: str):
        ...
        self._alert_thresholds: Dict[MetricType, Dict[AlertLevel, float]] = {
            MetricType.LATENCY: {
                AlertLevel.WARNING: 0.010,   # 10ms
                AlertLevel.ERROR: 0.050,     # 50ms
                AlertLevel.CRITICAL: 0.100   # 100ms
            },
            MetricType.ERROR_RATE: {
                AlertLevel.WARNING: 0.01,    # 1%
                AlertLevel.ERROR: 0.05,      # 5%
                AlertLevel.CRITICAL: 0.10    # 10%
            },
    ...
    def _trigger_alert(self, metric_type: MetricType, level: AlertLevel, 
    ...
    def get_metric_summary(self, metric_type: MetricType, window_seconds: float = 60.0) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            # 获取时间窗口内的样本
            recent_samples = [
                sample for sample in self._metrics[metric_type]
...
class PerformanceMonitor:
    ...
    def record_operation_end(self, operation_id: str, start_time: float, 
                           byte_count: int = 0, success: bool = True):
        """
        记录操作结束
    ...
    def generate_performance_report(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            for metric_type in MetricType:
                report['metric_summaries'][metric_type.value] = \
                    self._metrics_collector.get_metric_summary(metric_type)
