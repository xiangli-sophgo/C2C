...
class MessageSyncManager:
    ...
    def register_message_handler(self, message_type: SyncMessageType, 
    ...
    def send_sync_message(self, session_id: str, message_type: SyncMessageType,
                         payload: Dict[str, Any] = None, transaction_id: str = None) -> bool:
        """
        发送同步消息
    ...
    def _find_or_create_session_for_message(self, message: SyncMessage) -> Optional[str]:
        ...
        for session_id, session in self._sync_sessions.items():
            if (session.sender_chip_id == message.receiver_chip_id and 
                session.receiver_chip_id == message.sender_chip_id):
                return session_id
        ...
        session = SyncSession(
            session_id=session_id,
            sender_chip_id=message.receiver_chip_id,
            receiver_chip_id=message.sender_chip_id,
            state=SyncState.IDLE,
            created_time=time.time(),
            last_activity_time=time.time()
        )
    ...
    def _send_ack_message(self, session_id: str, original_message: SyncMessage):
        ...
        self.send_sync_message(
            session_id=session_id,
            message_type=SyncMessageType.ACK,
            payload=ack_payload,
            transaction_id=original_message.transaction_id
        )
    ...
    def get_sync_statistics(self) -> Dict[str, Any]:
        """获取同步统计信息"""
        with self._lock:
            active_sessions = len([s for s in self._sync_sessions.values() 
                                 if s.state not in [SyncState.SYNCED, SyncState.ERROR, SyncState.TIMEOUT]])
