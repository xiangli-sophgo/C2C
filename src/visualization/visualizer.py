...
class TopologyVisualizer:
    ...
    def __init__(self, figsize=(12, 8)):
        ...
        self.fig = None
        self.ax = None
        self.node_colors = {"chip": "#4CAF50", "switch": "#2196F3", "host": "#FF9800", "root": "#9C27B0"}  # 绿色 - 芯片  # 蓝色 - 交换机  # 橙色 - 主机  # 紫色 - 根节点
        self.link_colors = {"c2c": "#F44336", "pcie": "#607D8B", "default": "#757575"}  # 红色 - C2C链路  # 灰蓝色 - PCIe链路  # 灰色 - 默认链路
    ...
    def _draw_nodes(self, G, pos):
        ...
        for node_type in self.node_colors:
            nodes = [n for n, d in G.nodes(data=True) if d.get("node_type", "").startswith(node_type)]
            if nodes:
                nx.draw_networkx_nodes(G, pos, nodelist=nodes, node_color=self.node_colors[node_type], node_size=800, alpha=0.8, ax=self.ax)

    def _draw_edges(self, G, pos):
        """绘制边"""
        for link_type in self.link_colors:
            edges = [(u, v) for u, v, d in G.edges(data=True) if d.get("link_type", "default") == link_type]
            if edges:
                nx.draw_networkx_edges(G, pos, edgelist=edges, edge_color=self.link_colors[link_type], width=2, alpha=0.6, ax=self.ax)

    def _draw_labels(self, G, pos):
        """绘制节点标签"""
        labels = {node: node.replace("_", "\n") for node in G.nodes}
        nx.draw_networkx_labels(G, pos, labels, font_size=8, font_weight="bold", ax=self.ax)
    ...
    def _add_legend(self):
        ...
        for node_type, color in self.node_colors.items():
            legend_elements.append(patches.Patch(color=color, label=f"{node_type.title()}节点"))

        # 链路类型图例
        for link_type, color in self.link_colors.items():
            if link_type != "default":
                legend_elements.append(patches.Patch(color=color, label=f"{link_type.upper()}链路"))
    ...
    def _visualize_2d_torus(self, torus_structure, **kwargs):
        ...
        for chip_id, (x, y) in coord_map.items():
            # 绘制芯片节点
            circle = patches.Circle((x, y), 0.3, color=self.node_colors["chip"], alpha=0.8)
            self.ax.add_patch(circle)

            # 添加芯片ID标签
            self.ax.text(x, y, str(chip_id), ha="center", va="center", fontsize=8, fontweight="bold")
        ...
        for chip_id, neighbor_dict in neighbors.items():
            ...
            for direction, neighbor_coord in neighbor_dict.items():
                ...
                # 绘制直线连接
                self.ax.plot([x1, x2], [y1, y2], color=self.link_colors["c2c"], linewidth=2, alpha=0.6)
        ...
        self.ax.text(0.02, 0.98, info_text, transform=self.ax.transAxes, verticalalignment="top", bbox=dict(boxstyle="round", facecolor="wheat", alpha=0.8))
    ...
    def _visualize_3d_torus(self, torus_structure, **kwargs):
        ...
        for chip_id, (proj_x, proj_y) in pos_2d.items():
            ...
            circle = patches.Circle((proj_x, proj_y), 0.3, color=self.node_colors["chip"], alpha=alpha)
            self.ax.add_patch(circle)

            # 添加标签，包含3D坐标
            self.ax.text(proj_x, proj_y, f"{chip_id}\n({x},{y},{z})", ha="center", va="center", fontsize=6, fontweight="bold")
        ...
        self.ax.text(0.02, 0.98, info_text, transform=self.ax.transAxes, verticalalignment="top", bbox=dict(boxstyle="round", facecolor="lightblue", alpha=0.8))
