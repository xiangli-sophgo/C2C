...
class VisualizationStyleManager:
    ...
    def __init__(self, color_manager: Optional[ColorManager] = None):
        ...
        # E-Tag样式映射 - 统一定义，避免重复
        self.ETAG_STYLES = {"T0": {"line_width": 2.0, "edge_color": "darkred"}, "T1": {"line_width": 1.5, "edge_color": "darkblue"}, "T2": {"line_width": 1.0, "edge_color": "black"}}

        # 透明度配置
        self.ALPHA_CONFIG = {"default": 1.0, "min": 0.4, "step": 0.2, "tags_mode": 0.3}  # 默认透明度  # 最小透明度  # 透明度递减步长  # 标签模式透明度

    def get_flit_style(
        self, flit: Any, use_highlight: bool = False, expected_packet_id: Any = None, highlight_color: Optional[str] = None, show_tags_mode: bool = False
        ...
        # 获取基础颜色
        base_color = self.color_manager.get_packet_color(attrs["packet_id"], use_highlight, expected_packet_id, show_tags_mode)
        ...
        return (face_color_rgba, etag_style["line_width"], etag_style["edge_color"])
    ...
    def _calculate_alpha(self, attrs: Dict[str, Any], show_tags_mode: bool) -> float:
        ...
        if flit_id is not None:
            try:
                ...
                alpha = max(self.ALPHA_CONFIG["min"], self.ALPHA_CONFIG["default"] - (int(flit_id) * self.ALPHA_CONFIG["step"]))
                return alpha
            except (ValueError, TypeError):
                pass
    ...
    def set_etag_style(self, etag_priority: str, line_width: float, edge_color: str):
        ...
        Args:
            ...
            edge_color: 边框颜色
        """
        self.ETAG_STYLES[etag_priority] = {"line_width": line_width, "edge_color": edge_color}
    ...
    def apply_style_to_patch(self, patch, flit: Any, use_highlight: bool = False, expected_packet_id: Any = None, highlight_color: Optional[str] = None, show_tags_mode: bool = False):
        """
        ...
        # 获取样式
        face_color, line_width, edge_color = self.get_flit_style(flit, use_highlight, expected_packet_id, highlight_color, show_tags_mode)
    ...
    def create_empty_patch_style(self) -> Dict[str, Any]:
        ...
        Returns:
            样式字典
        """
        return {"facecolor": "none", "edgecolor": "gray", "linewidth": 0.8, "linestyle": "--", "alpha": 0.7}

    def __repr__(self) -> str:
        return f"VisualizationStyleManager(" f"etag_styles={len(self.ETAG_STYLES)}, " f"color_manager={self.color_manager})"
