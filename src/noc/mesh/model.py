...
class MeshPacket:
    ...
    def __init__(self, packet_id: PacketId, src: NodeId, dst: NodeId, 
                 size: int, injection_time: int, packet_type: str = "data"):
...
class MeshModel(BaseNoCModel):
    ...
    def get_network_statistics(self) -> Dict[str, Any]:
        ...
        # 计算平均值
        avg_latency = (self.stats["total_latency"] / total_packets 
                      if total_packets > 0 else 0)
        avg_hops = (self.stats["total_hops"] / total_packets 
                   if total_packets > 0 else 0)
        
        # 计算利用率（简化）
        total_possible_flits = self.current_cycle * self.mesh_config.num_nodes
        utilization = ((self.stats["total_flits_completed"] / total_possible_flits * 100) 
                      if total_possible_flits > 0 else 0)
        
        # 吞吐量
        throughput = (self.stats["total_packets_completed"] / self.current_cycle 
                     if self.current_cycle > 0 else 0)
    ...
    def get_all_router_statistics(self) -> Dict[NodeId, Dict[str, Any]]:
        """获取所有路由器的统计信息"""
        return {
            router_id: self.get_router_statistics(router_id)
