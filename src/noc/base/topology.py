...
from src.noc.utils.types import NodeId, Path, Position, NoCMetrics, RoutingStrategy, TopologyType, ValidationResult, AdjacencyMatrix, LinkId, MetricsDict, ConfigDict
...
class BaseNoCTopology(ABC):
    ...
    def calculate_route(self, src: NodeId, dst: NodeId, strategy: Optional[RoutingStrategy] = None) -> Path:
        ...
        # 检查缓存
        if strategy in self._routing_tables and src in self._routing_tables[strategy] and dst in self._routing_tables[strategy][src]:
        ...
        for src in range(self.num_nodes):
            ...
            for dst in range(self.num_nodes):
                if dst not in self._routing_tables[strategy][src]:
                    self._routing_tables[strategy][src][dst] = self.calculate_route(src, dst, strategy)
    ...
    def get_traffic_distribution(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_links": len([link for link, metrics in self._link_metrics.items() if metrics.get("utilization", 0) > 0]),
