...
class BaseFlit(ABC):
    ...
    def __repr__(self) -> str:
        """字符串表示"""
        status = []
        if self.is_injected: status.append("I")
        if self.is_arrive: status.append("A")
        if self.is_finish: status.append("F")
        if self.is_blocked: status.append("B")
        
        status_str = "".join(status) if status else "N"
        
        return (f"Flit({self.packet_id}.{self.flit_id}: "
                f"{self.source}->{self.destination}@{self.current_position}, "
                f"hop={self.hop_count}, {status_str})")
    ...
    def get_stats(self) -> Dict[str, int]:
        ...
        with self._lock:
            return {
                ...
                "flit_type": self.flit_class.__name__
            }
...
# 工厂函数
def create_flit(flit_class: type, 
               source: NodeId,
               destination: NodeId,
