...
class BaseResourceManager:
    ...
    def allocate(self, resource_id: str, amount: int = 1, **kwargs) -> bool:
        ...
        self.allocated[resource_id] = {
            "amount": amount,
            "allocated_time": kwargs.get("time", 0),
            "info": kwargs
        }
...
class BaseIPInterface(ABC):
    ...
    def _setup_clock_domain_fifos(self) -> None:
        ...
        self.l2h_fifos = {
            "req": deque(maxlen=l2h_depth),
            "rsp": deque(maxlen=h2l_depth),
            "data": deque(maxlen=l2h_depth)
        }
        
        self.h2l_fifos = {
            "req": deque(maxlen=h2l_depth),
            "rsp": deque(maxlen=h2l_depth),
            "data": deque(maxlen=h2l_depth)
        }
    ...
    def enqueue_request(self,
        ...
        self.active_requests[packet_id] = {
            "flit": flit,
            "created_time": self.current_cycle,
            "status": "created"
        }
    ...
    def _handle_received_data(self, data: BaseFlit) -> None:
        ...
        if data.packet_id not in self.completed_requests:
            self.completed_requests[data.packet_id] = {
                "req": req,
                "data_flits": [],
