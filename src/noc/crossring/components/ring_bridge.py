...
class RingBridge:
    ...
    def __init__(self, node_id: int, coordinates: Tuple[int, int], config: CrossRingConfig, logger: logging.Logger, topology=None):
        ...
        self.rb_out_depth = getattr(config, "RB_OUT_DEPTH", 8)
    ...
    def _create_input_fifos(self) -> Dict[str, Dict[str, PipelinedFIFO]]:
        ...
        return {
            channel: {
                direction: PipelinedFIFO(f"ring_bridge_in_{channel}_{direction}_{self.node_id}", depth=self.rb_in_depth)
    ...
    def _create_output_fifos(self) -> Dict[str, Dict[str, PipelinedFIFO]]:
        ...
        return {
            channel: {
                direction: PipelinedFIFO(f"ring_bridge_out_{channel}_{direction}_{self.node_id}", depth=self.rb_out_depth)
    ...
    def _get_ring_bridge_config(self) -> Tuple[List[str], List[str]]:
        ...
        if routing_strategy == "XY":
            input_sources = ["IQ_TU", "IQ_TD", "RB_TR", "RB_TL", "RB_TU", "RB_TD"]
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            "buffer_occupancy": {
                "ring_bridge_input_fifos": {
                    k: {d: len(v) for d, v in vv.items()}
                    for k, vv in self.ring_bridge_input_fifos.items()
                },
                "ring_bridge_output_fifos": {
                    k: {d: len(v) for d, v in vv.items()}
                    for k, vv in self.ring_bridge_output_fifos.items()
                },
            }
        }