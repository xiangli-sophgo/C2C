...
class CrossRingIPInterface:
    ...
    def __init__(self, 
        ...
        self.rn_tracker_count = {
            "read": config.tracker_config.rn_r_tracker_ostd,
            "write": config.tracker_config.rn_w_tracker_ostd
        }
        ...
        if ip_type.startswith("ddr"):
            self.sn_tracker_count = {
                "ro": config.tracker_config.sn_ddr_r_tracker_ostd,  # 读专用
                "share": config.tracker_config.sn_ddr_w_tracker_ostd  # 写共享
            }
            self.sn_wdb_count = config.sn_ddr_wdb_size
        elif ip_type.startswith("l2m"):
            self.sn_tracker_count = {
                "ro": config.tracker_config.sn_l2m_r_tracker_ostd,
                "share": config.tracker_config.sn_l2m_w_tracker_ostd
            }
    ...
    def _move_pre_to_fifo(self) -> None:
        ...
        for channel in ["req", "rsp", "data"]:
            # L2H预缓冲区 → L2H FIFO
            if (self.l2h_pre_buffers[channel] is not None and 
                len(self.l2h_fifos[channel]) < self.l2h_fifos[channel].maxlen):
                self.l2h_fifos[channel].append(self.l2h_pre_buffers[channel])
                self.l2h_pre_buffers[channel] = None
            
            # H2L预缓冲区 → H2L FIFO  
            if (self.h2l_pre_buffers[channel] is not None and
                len(self.h2l_fifos[channel]) < self.h2l_fifos[channel].maxlen):
    ...
    def _inject_to_l2h_pre(self, channel: str) -> None:
        ...
        Args:
            channel: 通道名称
        """
        if (not self.inject_fifos[channel] or 
            len(self.l2h_fifos[channel]) >= self.l2h_fifos[channel].maxlen or
            self.l2h_pre_buffers[channel] is not None):
            return
    ...
    def _handle_received_request(self, req: CrossRingFlit) -> None:
        ...
        if req.req_type == "read":
            ...
            if req.req_attr == "new":
                ...
                if (self.sn_tracker_count["share"] > 0 and 
    ...
    def _process_waiting_requests(self, req_type: str, tracker_type: str) -> None:
        ...
        if req_type == "write":
            # 检查tracker和wdb资源
            if (self.sn_tracker_count[tracker_type] > 0 and 
                self.sn_wdb_count > 0):
