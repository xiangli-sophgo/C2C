...
from .base.config import BaseNoCConfig, CrossRingCompatibleConfig
...
class NoCTopologyFactory:
    ...
    @classmethod
    def register_topology(cls, topology_type: TopologyType, 
                         topology_class: Type[BaseNoCTopology],
                         config_creator: Optional[Callable[..., BaseNoCConfig]] = None,
                         validator: Optional[TopologyValidator] = None) -> None:
        """
        注册新的拓扑类型。
    ...
    @classmethod
    def create_topology(cls, config: BaseNoCConfig, 
                       validate: bool = True) -> BaseNoCTopology:
        """
        根据配置创建拓扑实例。
        ...
        if topology_type not in cls._topology_registry:
            ...
            raise TopologyCreationError(
                f"不支持的拓扑类型: {topology_type}. "
                f"支持的类型: {[t.value for t in supported_types]}"
            )
    ...
    @classmethod
    def create_config(cls, topology_type: TopologyType, 
    ...
    @classmethod
    def create_topology_from_type(cls, topology_type: TopologyType, 
                                 **kwargs) -> BaseNoCTopology:
        """
        根据拓扑类型和参数创建拓扑。
    ...
    def list_topologies(cls) -> Dict[str, Dict[str, Any]]:
        ...
        """
        return {
            topology_type.value: cls.get_topology_info(topology_type)
            for topology_type in cls._topology_registry
        }
    
    @classmethod
    def clone_topology(cls, original: BaseNoCTopology, 
                      config_updates: Optional[Dict[str, Any]] = None) -> BaseNoCTopology:
        """
        克隆现有拓扑。
    ...
    @classmethod
    def optimize_topology(cls, config: BaseNoCConfig, 
                         optimization_target: str = "latency") -> BaseNoCConfig:
        """
        优化拓扑配置。
...
def create_mesh_topology(width: int, height: int, **kwargs) -> BaseNoCTopology:
    ...
    return NoCTopologyFactory.create_topology_from_type(
        TopologyType.MESH,
        num_nodes=num_nodes,
        **kwargs
    )
...
def create_crossring_topology(num_node: int = 20, num_col: int = 2, 
                             num_ip: int = 16, **kwargs) -> BaseNoCTopology:
    """
    创建CrossRing拓扑的便捷函数。
