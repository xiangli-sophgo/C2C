...
class WindowManager:
    ...
    def _update_congestion_window(self, packet_acked: bool):
        ...
        # 限制窗口大小范围
        self._congestion_window = max(self._min_window_size, min(self._max_window_size, int(self._congestion_window)))
...
class FlowController:
    ...
    def packet_acknowledged(self, sequence_number: int, send_timestamp: float, receive_timestamp: float) -> bool:
        """
        记录包确认
    ...
    def receive_packet(self, data: bytes, source_id: str, sequence_number: int) -> bool:
        ...
        """
        with self._lock:
            entry = BufferEntry(data=data, timestamp=time.time(), source_id=source_id, sequence_number=sequence_number, size=len(data))
    ...
    def _update_bandwidth_stats(self, packet_size: int):
        ...
        # 添加带宽样本
        self._bandwidth_samples.append({"size": packet_size, "timestamp": current_time})

        # 计算当前带宽（基于最近1秒的数据）
        recent_samples = [s for s in self._bandwidth_samples if current_time - s["timestamp"] <= 1.0]
