...
try:
    from src.protocol.cdma_system import CDMAPacket
except ImportError:
    # 如果导入失败，使用简化的消息结构
    CDMAPacket = Any
...
class SimulationEvent:
    ...
    def __str__(self):
        """字符串表示"""
        return (f"事件[{self.event_type.value}] "
                f"时间:{self.timestamp_ns}ns "
                f"从:{self.source_chip_id} "
                f"到:{self.target_chip_id}")
...
class EventFactory:
    ...
    @staticmethod
    def create_cdma_send_event(timestamp_ns: int, source_chip_id: str, 
                              target_chip_id: str, data_size: int = 1024,
                              priority: int = 0) -> SimulationEvent:
        """创建CDMA发送事件"""
    ...
    @staticmethod
    def create_cdma_receive_event(timestamp_ns: int, source_chip_id: str,
                                 target_chip_id: str, cdma_packet: Optional[CDMAPacket] = None) -> SimulationEvent:
        """创建CDMA接收事件"""
        return SimulationEvent(
            timestamp_ns=timestamp_ns,
            event_type=EventType.CDMA_RECEIVE,
            source_chip_id=source_chip_id,
            target_chip_id=target_chip_id,
            event_id="",
            cdma_packet=cdma_packet
        )
    
    @staticmethod
    def create_link_transfer_event(timestamp_ns: int, source_chip_id: str,
                                  target_chip_id: str, data_size: int) -> SimulationEvent:
        """创建链路传输事件"""
        return SimulationEvent(
            timestamp_ns=timestamp_ns,
            event_type=EventType.LINK_TRANSFER,
            source_chip_id=source_chip_id,
            target_chip_id=target_chip_id,
            event_id="",
            data_size=data_size
