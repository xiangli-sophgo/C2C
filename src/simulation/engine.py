...
from src.protocol.cdma_system import CDMASystem
...
class C2CSimulationEngine:
        ...
        for node_id, node in self.topology_builder._nodes.items():
            if hasattr(node, 'chip_id'):  # 检查是否为ChipNode
                fake_chip = FakeChip(
                    ...
                    cdma_engines=node.cdma_engines
                )
    ...
    def add_cdma_send_event(self, timestamp_ns: int, source_chip_id: str, 
                           target_chip_id: str, data_size: int = 1024, priority: int = 0):
        """
        添加CDMA发送事件的便捷方法
        ...
        Args:
            ...
            priority: 事件优先级
        """
        event = EventFactory.create_cdma_send_event(
            timestamp_ns, source_chip_id, target_chip_id, data_size, priority
        )
        self.add_simulation_event(event)
        print(f"添加CDMA发送事件: {source_chip_id} -> {target_chip_id}, "
              f"时间: {timestamp_ns}ns, 大小: {data_size}字节")
    
    def add_periodic_traffic(self, source_chip_id: str, target_chip_id: str,
                           period_ns: int, data_size: int, start_time_ns: int = 0,
        ...
        print(f"添加周期性流量: {source_chip_id} -> {target_chip_id}, "
              f"周期: {period_ns}ns, 事件数: {event_count}")
    ...
    def run_simulation(self, simulation_time_ns: int) -> SimulationStats:
        ...
        try:
            ...
            end_event = SimulationEvent(
                ...
                source_chip_id="engine",
                target_chip_id="all",
                event_id="simulation_end"
            )
    ...
    def _generate_follow_up_events(self, event: SimulationEvent):
        ...
        if event.event_type == EventType.CDMA_SEND:
            # CDMA发送事件后，生成对应的接收事件
            # 计算传输延迟
            transfer_delay_ns = self._calculate_transfer_delay(
                event.source_chip_id, event.target_chip_id, event.data_size
            )
            
            receive_event = EventFactory.create_cdma_receive_event(
                timestamp_ns=event.timestamp_ns + transfer_delay_ns,
                source_chip_id=event.source_chip_id,
                target_chip_id=event.target_chip_id,
                cdma_packet=event.cdma_packet
            )
    ...
    def get_simulation_results(self) -> Dict[str, Any]:
        ...
        results = {
            ...
            "topology_info": {
                "topology_id": self.topology_graph.topology_id,
                "node_count": len(self.topology_builder._nodes),
                "link_count": len(self.topology_builder._links)
            },
