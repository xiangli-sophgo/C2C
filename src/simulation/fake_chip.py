...
class FakeChip(ChipNode):
    ...
    def _handle_cdma_send_event(self, event: SimulationEvent):
        """处理CDMA发送事件"""
        print(f"芯片 {self.chip_id} 发送CDMA消息到 {event.target_chip_id}, "
              f"数据大小: {event.data_size} 字节")
    ...
    def __str__(self):
        ...
        return (f"FakeChip({self.chip_id}) - "
                f"发送: {stats['sent_messages']} 消息, "
                f"接收: {stats['received_messages']} 消息, "
                f"时间: {stats['current_time_ns']} ns")