...
from topology.torus import TorusTopologyLogic, test_torus_connectivity
...
def validate_tree_topology_demo():
    ...
    for chip_count in test_cases:
        ...
        try:
            ...
            tree_root, all_nodes = topo_logic.calculate_tree_structure(
                list(range(chip_count)), switch_capacity=8
            )
...
def validate_torus_topology_demo():
    ...
    for chip_count, dimensions in test_cases:
        ...
        try:
            ...
            # 测试路由算法
            from topology.torus import TorusRoutingLogic
            ...
            for src_id, dst_id in test_pairs:
                if dst_id < chip_count:
                    ...
                    path = routing_logic.dimension_order_routing(
                        src_coord, dst_coord, torus_structure['grid_dimensions']
                    )
...
def create_tree_topology_with_framework():
    ...
    try:
        ...
        for i in range(8):
            chip = ChipNode(
                chip_id=f"chip_{i}",
                board_id=f"board_{i//4}",  # 每4个芯片一个板卡
                memory_types=["DDR", "HBM"]
            )
            chips.append(chip)
        ...
        # 连接芯片到交换机
        for i in range(4):
            builder.add_link(C2CDirectLink(
                link_id=f"link_chip{i}_switch0",
                endpoint_a=chips[i],
                endpoint_b=switch1
            ))
        ...
        for i in range(4, 8):
            builder.add_link(C2CDirectLink(
                link_id=f"link_chip{i}_switch1",
                endpoint_a=chips[i],
                endpoint_b=switch2
            ))
        ...
        # 连接交换机到根交换机
        builder.add_link(C2CDirectLink(
            link_id="link_switch0_root",
            endpoint_a=switch1,
            endpoint_b=root_switch
        ))
        
        builder.add_link(C2CDirectLink(
            link_id="link_switch1_root",
            endpoint_a=switch2,
            endpoint_b=root_switch
        ))
        ...
        for chip_id, (x, y) in chip_coords.items():
            ...
            for nx, ny in neighbors:
                ...
                if neighbor_id > chip_id:  # 避免重复连接
                    ...
                    builder.add_link(C2CDirectLink(
                        link_id=link_id,
                        endpoint_a=chips[chip_id],
                        endpoint_b=chips[neighbor_id]
                    ))
...
def test_cdma_communication(topology):
    ...
    try:
        ...
        # 创建测试消息
        test_message = CDMAMessage(
            source_id="chip_0",
            destination_id="chip_7",
            message_type="send",
            payload="测试数据从芯片0发送",
