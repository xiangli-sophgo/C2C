...
from utils.constants import (
    DEFAULT_PCIE_X8_BANDWIDTH,
    DEFAULT_PCIE_X4_BANDWIDTH,
    DEFAULT_C2C_BANDWIDTH,
    PCIE_BASE_LATENCY,
    C2C_BASE_LATENCY
)
...
class PCIeLink(BaseLink):
    ...
    def bandwidth(self) -> float:
        ...
        if pcie_type == "x8":
            return DEFAULT_PCIE_X8_BANDWIDTH
        elif pcie_type == "x4":
            return DEFAULT_PCIE_X4_BANDWIDTH
        else:
    ...
    def latency(self) -> float:
        """PCIe链路延迟"""
        return PCIE_BASE_LATENCY # + transmission delay (simplified for now)
...
class C2CDirectLink(BaseLink):
    ...
    def bandwidth(self) -> float:
        """C2C直连链路带宽"""
        return DEFAULT_C2C_BANDWIDTH

    @property
    def latency(self) -> float:
        """C2C直连链路延迟"""
        return C2C_BASE_LATENCY
